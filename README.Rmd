...
Note that all `ggplot2::theme()` settings inherent in `dsb_theme*()` can be overwritten by adding `theme(...)` elements after.
...
* When mapping `color_dsb_d()` or `fill_dsb_d()` to a variable with __only two levels__, you can manually choose colors with the `primary` and `secondary` arguments. As with `epinionDSB::dsb_grabcol()` colors can be specified with or without the DSB prefix
